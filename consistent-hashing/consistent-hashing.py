...
class ConsistentHashing:
    ...
    def add_server(self, server):
        ...
        for i in range(self.num_replicas):  # Creating multiple virtual nodes
            hash_val = ConsistentHashing._hash(f"{server}-{i}")  # Unique hash for each virtual node
    ...
    def remove_server(self, server):
        ...
        if server in self.servers:
            ...
            for i in range(self.num_replicas):
                hash_val = ConsistentHashing._hash(f"{server}-{i}")  # Remove each virtual node's hash
    ...
    def get_server(self, key):
        ...
        hash_val = ConsistentHashing._hash(key)  # Hash the key
